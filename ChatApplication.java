...
public static void main(String args[])
{
	...
	box =new JComboBox(new String[]{"None","Anonymous"});    
              ...
              try
              {
              	if(friends.getSelectedValue()!=null)
              	{
              		...
              		if(!group.containsKey(friends.getSelectedValue().toString()))
              		{
						...
						if(box.getItemAt(box.getSelectedIndex()).toString().equals("Anonymous"))
								packet = new Packet("Anonymous",new String[]{recipient},message.getText(),"message","");
              			else
								packet = new Packet(user.getText(),new String[]{recipient},message.getText(),"message","");
              			chat.append(str);
    ...
    lsm.addListSelectionListener(new ListSelectionListener() {
            ...
            public void valueChanged(ListSelectionEvent arg) {
				...
				if(queue.size()>0)
				{
					model.removeElement(queue.get(0));
...
public void run()
{
	try
	{
		...
		while((packet=(Packet)is.readObject())!=null)
		{
			...
			{
				if(!friends.getSelectedValue().toString().equals(packet.getSender()))
					model.removeElement(packet.getSender());
				else
					queue.add(packet.getSender());
				//msg.put(packet.getSender(),new StringBuilder(""));
			}
			...
			{
				if(packet.getSender().equals("Anonymous"))
					chat.append("From "+packet.getSender()+": "+packet.getMessage()+"\n");
				else
				{	
				String msg_in = "From "+packet.getSender()+": "+packet.getMessage()+"\n";
				...
				if(recipientCount==1)
				{
					if(!msg.containsKey(packet.getSender()))
					{
						msg.put(packet.getSender(),new StringBuilder(msg_in));
					}
					else
					{
						StringBuilder data = msg.get(packet.getSender());
						boolean bool = msg.replace(packet.getSender(),data,data.append(msg_in));
						if(!bool)
							throw new Exception();
					}
				...
				else if(recipientCount>1)
				{
					...
					{
						msg.put(gname,new StringBuilder(msg_in));
					}
					else
					{
						StringBuilder data = msg.get(gname);
						boolean bool = msg.replace(gname,data,data.append(msg_in));
						if(!bool)
							throw new Exception();
					}
					if(packet.getSender().equals("Server"))
